...
class DataProcessor(object):
  ...
  # @classmethod
  # def _load_tsv(cls, input_file, s1_idx=0, s2_idx=1, targ_idx=2, idx_idx=None, quotechar=None):
  #   sent1s, sent2s, targs, idxs = [], [], [], []
  #   with tf.gfile.Open(input_file, "r") as f:
  #     lines = csv.reader(f, delimiter="\t", quotechar=quotechar)
  #     for line in lines:
  #       sent1s.append(line[s1_idx])
  #       if s2_idx is not None:
  #         sent2s.append(line[s2_idx])
  #       if targ_idx is not None:
  #         targs.append(line[targ_idx])
  #       if idx_idx is not None:
  #         idxs.append(line[idx_idx])
...
class IMDBProcessor(DataProcessor):
  ...
  def preprocess_text(self, sen):
    ...
    return sentence
  def _random_gen_and_record(self, fn='rd.txt', n=50000):
    random_arr = []
    if os.path.isfile(fn):
      with open(fn, 'r') as file:
        ...
        for num in nums:
          if(num.isnumeric()):
            random_arr.append(int(num))
        # print("size of random_arr in _random_gen_and_record(): %d" % len(random_arr))
        return random_arr
    ...
    with open(fn, 'w') as file:
      for random_num in random_arr:
        if not text:
          ...
          text = "%s,%d" % (text, random_num)
      file.write(text)
    return random_arr
  
  def _get_ex_from_csv(self, data_file='/content/mnt/My Drive/workspace/data/glue/IMDB Dataset.csv'):
    movie_reviews = pd.read_csv(data_file)
    reviews = []
    sentences = list(movie_reviews['review'])
    for sen in sentences:
      reviews.append(self.preprocess_text(sen))
    labels = list(movie_reviews["sentiment"])
    examples = []
    i = 0
    for (label, review) in zip(labels, reviews):
      i = i + 1
      guid = i
      text_a = tokenization.convert_to_unicode(review)
      label = tokenization.convert_to_unicode(label)
      # print("text_a:\n")
      # print("%s\n" % text_a)
      # print("label: %d\n" % label)
      examples.append(InputExample(guid=guid, text_a=text_a, label=str(label)))
    return examples
  ...
  def _create_examples(self, set_type):
    ...
    random_arr = self._random_gen_and_record()
    # print("size of random_arr: %d" % len(random_arr))
    examples = self._get_ex_from_csv()
    # print("size of examples: %d" % len(examples))
    batches = {
        'train': random_arr[:45000],
        'dev': random_arr[45000:],
    }
    ex_batch = []
    # print("size of %s batch: %d" % (set_type, len(ex_batch)) )
    for ex_num in batches[set_type]:
      # print("example check:")
      # print(examples[ex_num])
      ex_batch.append(examples[ex_num])
    # print("size of ex_batch: %d" % len(ex_batch) )
    return ex_batch
...
class QQPProcessor(DataProcessor):
  """Processor for Quora Question Pairs."""
...
class SNLIProcessor(DataProcessor):
  """Processor for Stanford Natural Language Inference."""
...
class MNLIProcessor(DataProcessor):
  """Processor for Multi-Genre Natural Language Inference."""
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    m_dev_examples = self._create_examples(self._read_tsv(os.path.join(data_dir, "dev_matched.tsv")),
        "dev_matched")
  ...
  def get_test_examples(self, data_dir):
    """See base class."""
    m_test_examples = self._create_examples(self._read_tsv(os.path.join(data_dir, "test_matched.tsv")),
        "test_matched")
    mm_test_examples = self._create_examples(self._read_tsv(os.path.join(data_dir, "test_mismatched.tsv")),
        "test_mismatched")
...
class SSTProcessor(DataProcessor):
  """Processor for Stanford Sentiment Treebank."""
...
class RTEProcessor(DataProcessor):
  """Processor for Recognizing Textual Entailment."""
...
class QNLIv2Processor(DataProcessor):
  """Processor for SQuAD NLI."""
...
class WNLIProcessor(DataProcessor):
  """Processor for Winograd NLI task."""
